...
# DESC: Checks whether the container is initialized or not. If not, initializes it. After that run both httpd and php-fpm.
main() {
  if [[ ! -e "${INITIALIZED_FILE_PATH}" ]]; then
    echo "Initialize PHP-FPM-APACHE container..."
    initialize
  else
    echo "PHP-FPM-APACHE container is already initialized."
  fi
