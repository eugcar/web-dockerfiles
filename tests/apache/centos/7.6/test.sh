...
function script_init() {
  readonly CONTAINER_NAME=www_centos76_test
  readonly CONTAINER_TO_BUILD=ecarocci/base:centos7.6
  readonly CONTAINER_TO_TEST=ecarocci/www:centos7.6
...
function build_required_images() {
  cd ../../../..
  build_image "${CONTAINER_TO_BUILD}" ./images/base/centos/7.6/base/Dockerfile
  build_image "${CONTAINER_TO_TEST}"  ./images/apache/centos/7.6/Dockerfile
  cd -  > /dev/null 2>&1
}
