...
function script_init() {
  readonly CONTAINER_NAME=php_fpm_symfony_3_74_test
  readonly CONTAINER_TO_BUILD_0=ecarocci/base:centos7.7
  readonly CONTAINER_TO_BUILD_1=ecarocci/base:centos7.7-remi
  readonly CONTAINER_TO_BUILD_2=ecarocci/php-fpm-base:7.4
  readonly CONTAINER_TO_TEST=ecarocci/php-fpm-symfony-3:7.4
...
function build_required_images() {
  ...
  build_image "${CONTAINER_TO_BUILD_2}" ./images/php/7.4/php-fpm/base/Dockerfile
  build_image "${CONTAINER_TO_TEST}"    ./images/php/7.4/php-fpm/symfony-3/Dockerfile
  cd -  > /dev/null 2>&1
}
