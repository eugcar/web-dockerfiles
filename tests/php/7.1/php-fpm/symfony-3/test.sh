...
function script_init() {
  readonly CONTAINER_NAME=php_fpm_symfony_3_71_test
  readonly CONTAINER_TO_BUILD_0=ecarocci/base:centos7.6
  readonly CONTAINER_TO_BUILD_1=ecarocci/base:centos7.6-ius
  readonly CONTAINER_TO_BUILD_2=ecarocci/php-fpm-base:7.1
  readonly CONTAINER_TO_TEST=ecarocci/php-fpm-symfony-3:7.1
...
function build_required_images() {
  ...
  build_image "${CONTAINER_TO_BUILD_2}" ./images/php/7.1/php-fpm/base/Dockerfile
  build_image "${CONTAINER_TO_TEST}"    ./images/php/7.1/php-fpm/symfony-3/Dockerfile
  cd -  > /dev/null 2>&1
}
...
function execute_tests() {
  ...
  docker exec "${CONTAINER_NAME}" bash -c ". ${PATH_TO_CREATE_SYMFONY_3_PARAMETERS_FILE_FUNCTION} && assertSymfony3ParametersFileIsPresent ${PARAMETERS_FILE_MD5_SUM_TO_CHECK}"
  ...
  docker exec "${CONTAINER_NAME}" bash -c ". ${PATH_TO_START_PHPFPM_FUNCTION} && assertIsPhpFpmRunning /run/php-fpm/php-fpm.pid"
}
